...
class MD5_plink:
    ...
    def __init__(self, geno_path, allele_list='GP2_allcall_snps.set'):
    ...
    def allele_string_traw_gen(self, traw_path, samples_to_search):
        ...
        for sample_for_search in samples_to_search:
            ...
            if len(traw) != len(fingerprint_list):
                traw_merged = fingerprint_list.merge(traw.drop(columns = 'CHR'), how = "outer", on = "POS")
                warning_alleles = list()
                for _, row in traw_merged.fillna(-1).iterrows():
                    val = row.iloc[sampleColumn]
                    if val >= 0 and val < 0.5:
                        allele = row.loc['ALT'] + row.loc['ALT']
                    elif val >= 0.5 and val < 1.5:
                        allele = row.loc['COUNTED'] + row.loc['ALT']
                    elif val >= 1.5:
                        allele = row.loc['COUNTED'] + row.loc['COUNTED']
                    else:
                        allele = 'NN'
                        warning_alleles.append(str(row.iloc[0])+':'+str(row.iloc[1]))
                    string_traw = string_traw+allele

                warnings[sample_for_search] = warning_alleles
                all_missing_alleles = all_missing_alleles + warning_alleles

            # If the number of alleles are identical, then no warning message
            else:
                for _, row in traw.iterrows():
                    val = row.iloc[sampleColumn]
                    if val >= 0 and val < 0.5:
                        allele = row.loc['ALT']+row.loc['ALT']
                    elif val >= 0.5 and val < 1.5:
        ...
        with open(f'{traw_path}_missing_alleles.txt', 'w') as f:
    ...
    def allele_string_gen(self):
        ...
        pd.DataFrame({'sampleID':sampleID, 'hash':allele_hash}, dtype=str).to_csv(f'{geno_path}_MD5_hash.txt', sep='\t', header=True, index=False)
